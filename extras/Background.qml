...
Item {
  ...
  readonly property bool matrixRainActive: themeName.trim().toLowerCase() === "matrix"

  FileView {
    id: themeNameFile
    path: root.currentThemeNamePath
    watchChanges: true
    onLoaded: root.themeName = text()
  ...
  Variants {
    ...
    PanelWindow {
      ...
      Canvas {
        id: matrixRain
        anchors.fill: parent
        visible: root.matrixRainActive
        z: 100
        renderStrategy: Canvas.Cooperative

        readonly property real cellSize: 20
        readonly property string glyphs: "アイウエオカキクケコサシスセソタチツテトナニヌネノハヒフヘホマミムメモヤユヨラリルレロワヲン0123456789"
        property var drops: []
        ...
        function reset() {
          columnCount = Math.max(1, Math.ceil(width / cellSize))
          var next = []
          for (var i = 0; i < columnCount; i++) next.push(Math.floor(Math.random() * (height / cellSize)) - columnCount)
          drops = next
        ...
        onPaint: {
          ...
          for (var i = 0; i < drops.length; i++) {
            var y = drops[i] * cellSize
            if (y >= 0 && y <= height) ctx.fillText(glyph(), i * cellSize + 2, y)
            drops[i]++
